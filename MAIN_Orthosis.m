...
% coarse grid
cg_res = [1,1,1]*4.1; % spacing between grid points in xyz direction [mm]

% input file
name = ['src',filesep,'01_in',filesep,'Orthosis_Full.stl']; % name and path to .stl input file
type = 'shell';  % type of file ('shell' or 'solid')

% geometry output
FEinfo.ElementSettings.ShellThickness = 3; % thickness of elements if type='shell'
FEinfo.ElementSettings.ElemSize = 4.1; % target edge length of FE elements
...
matinfo.basemat = [25,0.01,1234;25,1,900;25,86400,160];
...
operations{1} = library{31};
operations{2} = library{32};
operations{3} = library{40};
...
FEinfo.InpFile = 'Orthosis'; % name of Abaqus input file
...
% Generate FE Mesh
geomName = ['src',filesep,'01_in',filesep,'Orthosis_Full.stl']; % stl file containing full geometry
FEinfo.MeshImport = ''; % optional - name of external mesh to be imported
