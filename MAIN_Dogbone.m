...
% coarse grid
cg_res = [1,1,1]*0.1; % spacing between grid points in xyz direction [mm]

% input file
name = ['src',filesep,'01_in',filesep,'ASTM_D638_IV.stl']; % name and path to .stl input file
type = 'solid';  % type of file ('shell' or 'solid')

% geometry output
FEinfo.ElementSettings.ShellThickness = 1; % thickness of elements if type='shell'
FEinfo.ElementSettings.ElemSize = 1; % target edge length of FE elements
...
matinfo.basemat = [matinfo.mat1.ID, 0.835; matinfo.mat2.ID, 0.165];
...
operations{1} = library{11};
operations{2} = library{12};
operations{3} = library{13};
operations{4} = library{14};
...
FEinfo.InpFile = 'Dogbone'; % name of Abaqus input file
...
% Generate FE Mesh
geomName = ['src',filesep,'01_in',filesep,'ASTM_D638_IV.stl']; % stl file containing full geometry
