...
switch type
    case 'solid'
        gridOUTPUT = VOXELISE_SOLID(gridX,gridY,gridZ,meshXYZ);
    case 'shell'
        gridOUTPUT = VOXELISE_SHELL(gridX,gridY,gridZ,meshXYZ);
end
end


%% Solid Voxelizer
function [gridOUTPUT] = VOXELISE_SOLID(gridX,gridY,gridZ,meshXYZ)
...
%% Shell Voxelizer
function [gridOUTPUT] = VOXELISE_SHELL(gridX,gridY,gridZ,meshXYZ)
% grid size
nx = size(gridX,2);
ny = size(gridY,2);
nz = size(gridZ,2);
...
% load centroids
centroids = meshXYZ.centroids;

% make template for point IDs to fill
fillID = nan(size(centroids,1),8);
...
parfor i=1:size(centroids,1)
    ...
    % left grid indices
    IDx = find(gridX-pt(1)<0,1,'last');
    IDy = find(gridY-pt(2)<0,1,'last');
    IDz = find(gridZ-pt(3)<0,1,'last');
    
    % full set of grid indices
    gridID_1 = (IDz-1)*nx*ny + (IDy-1)*nx + IDx;
    gridID_2 = gridID_1 + 1;
    gridID_3 = gridID_1 + nx;
    gridID_4 = gridID_3 + 1;
    gridID_5 = gridID_1 + nx*ny;
    gridID_6 = gridID_5 + 1;
    gridID_7 = gridID_5 + nx;
    gridID_8 = gridID_7 + 1;  

    % load to template
    fillID(i,:) = [gridID_1,gridID_2,gridID_3,gridID_4,gridID_5,gridID_6,gridID_7,gridID_8];
end

% unique IDs to fille
fillID = unique(fillID(:));

% create voxel structure
gridvec         = logical(zeros(nx*ny*nz,1));
gridvec(fillID) =  1;

% reshape
gridOUTPUT  = reshape(gridvec,nx,ny,nz);
